...
function parseHtmlForQuote(html, target) {
  var parser = new DOMParser();
  var doc = parser.parseFromString(html, "text/html");
  var targets = doc.querySelector(target);
  var output = targets.innerHTML;
...
async function doWork() {
  ...
  var quoteUrl = "https://www.cnbc.com/quotes/" + ticker
  var quoteHtml = await getHTML(quoteUrl);
  var quote = parseHtmlForQuote(quoteHtml, '.QuoteStrip-lastPrice');
  ...
  var valnode = document.getElementById("currentValue");
  while (valnode.firstChild)
    valnode.removeChild(valnode.firstChild);
